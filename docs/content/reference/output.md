...
describes what users get. Pick a format with `-o`, or let discord2110 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
discord2110 <command> -o table   # aligned columns for reading
discord2110 <command> -o jsonl   # one JSON object per line, for piping
discord2110 <command> -o json    # a single JSON array
discord2110 <command> -o csv     # spreadsheet friendly
discord2110 <command> -o tsv     # tab-separated
discord2110 <command> -o url     # just the URL column
discord2110 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
discord2110 <command> --fields id,title,url
```
...
```bash
discord2110 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
discord2110 <command>            # a table, because this is a terminal
discord2110 <command> | wc -l    # JSONL, because this is a pipe
```
